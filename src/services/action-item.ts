...
export const update = async (props: Item) => {
  try{
    const updateQuery: string = `UPDATE items
      ...
      WHERE id = $1;`
    let updateValues: string[] = Object.values(props)
...
export const getAll = async (filter: { userId:string, completed:string, isActive:string }): Promise<Item[]> => {
  try {
    let getQuery: string = 'SElECT * FROM items WHERE user_id = $1 AND section_id = 2'
...
export const getOne = async (id: string): Promise<Item> => {
  try {
    const getQuery: string = `SElECT * FROM items WHERE id = $1`
