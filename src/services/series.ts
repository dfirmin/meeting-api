...
export const create = async (props: Series): Promise<string> => {
  try {
    const createQuery = `INSERT INTO series (admin_user_id, time_allocated, start_date, team_id) VALUES($1, $2, $3, $4) RETURNING id;`
    delete props.id
...
export const update = async (props: Series): Promise<void> => {
  try {
    const updateQuery = `UPDATE series SET admin_user_id = $2 time_allocated = $3, start_date = $4, team_id = $5 WHERE id = $1;`
